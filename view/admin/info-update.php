<?php
    ...
    if (!isset($_GET['ids'])) {
        ...
        header("Location: " . getBaseUrl() . "/admin?qlnguoidung");
        exit();
    }
    ...
    $result = $p->getupdateuser_with_password($id, $username, $email, $password, $phone, $address, $avatar, $vt);
        ...
        if ($result) {
            require_once __DIR__ . '/../../helpers/url_helper.php';
            header("Location: ?taikhoan&status=updated");
            exit();
        } else {
...
<head>
    ...
    <style>
        ...
        .info-update-container .card-header {
            background: white;
            border-bottom: 2px solid #f0f0f0;
            padding: 20px;
        }
        
        .info-update-container .card-title {
            font-size: 1.25rem;
            font-weight: 600;
            color: #333;
            margin: 0;
        ...
        .info-update-container .card-body {
            padding: 30px;
        }
    </style>
</head>
                            ...
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" id="role_id" name="role_id" required>
                                        <option value="2" <?php echo (isset($user[0]['role_id']) && $user[0]['role_id'] == 2) ? 'selected' : ''; ?>>Người dùng</option>
                                        <option value="1" <?php echo (isset($user[0]['role_id']) && $user[0]['role_id'] == 1) ? 'selected' : ''; ?>>Admin</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
